...
impl Document {
    ...
    /// Записывает в байты только одно поддерево.
    ///
    /// Нужно для сравнения узлов между собой: сериализованный вид учитывает
    /// всё, включая свойства, о которых вызывающий не знает. Сравнение
    /// разобранных полей поле за полем таких промахов не избежало бы.
    ///
    /// # Errors
    ///
    /// Те же, что у [`Document::serialize`].
    pub fn serialize_node(&self, n: NodeId) -> Result<Vec<u8>> {
        let mut out = Vec::new();
        self.write_node(n, &mut out, 0)?;
        Ok(out)
    }
